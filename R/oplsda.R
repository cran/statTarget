...
#library(car)
car::dataEllipse(T[,nf], Tortho[,1], levels = c(0.95), add=TRUE, col = "black", lwd = 0.4, plot.points=FALSE, center.cex=0.2)
dev.off()
...
pc.all <- prcomp(X, center=FALSE, scale=FALSE)
 ...
 #requireNamespace(car) 
 #library(car)
 car::dataEllipse(pc.all$x[,1], pc.all$x[,2], levels = c(0.95), add=TRUE, col = "black", lwd = 0.4, plot.points=FALSE, center.cex=0.2)
